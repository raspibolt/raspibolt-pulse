...
if [ "${gitupdate}" -eq "1" ]; then
  ...
  corelngit=$(curl -s https://api.github.com/repos/ElementsProject/lightning/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")')
  ...
  printf "%s\n%s\n%s\n%s\n%s\n" "${btcgit}" "${corelngit}" "${electrsgit}" "${btcrpcexplorergit}" "${rtlgit}" > "${gitstatusfile}"
else
  # read from file
  btcgit=$(sed -n '1p' < "${gitstatusfile}")
  #lndgit=$(sed -n '2p' < "${gitstatusfile}")
  corelngit=$(sed -n '2p' < "${gitstatusfile}")
...
coreln_running=$(systemctl is-active cln)
coreln_color="${color_green}"
if [ -z "${coreln_running##*inactive*}" ]; then
  coreln_running="down"
  coreln_color="${color_red}\e[7m"
else
  if [ -z "${coreln_running##*failed*}" ]; then
    coreln_running="down"
    coreln_color="${color_red}\e[7m"
  else
    coreln_running="up"
...
#get channel.db size
coreln_dir="/data/cln"
channel_db_size=$(du -h ${coreln_dir}/bitcoin/lightningd.sqlite3 | awk '{print $1}')
...
${color_yellow}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${color_green}     .~~.   .~~.      ${color_yellow}%-22s${bitcoind_color}%-4s${color_grey}   ${color_yellow}%-20s${coreln_color}%-4s
...
"${btc_title}" "${bitcoind_running}" "Lightning (CLN)" "${coreln_running}" \
